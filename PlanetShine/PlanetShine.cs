...
namespace PlanetShine {
	...
	public class PlanetShine : MonoBehaviour {
		...
		// Attributes for DEBUGGING purposes only
		...
		// Attributes which identify the body the vessel is currently orbiting body
		...
		// Data/Attributes for calculating and rendering albedo and ambient lighting created by Planetshine
		...
		public int albedoLightsQuantity {
			// Sepcial case for the Sun as we only want 1 albedo light to correctly handle the shadow.
			get {
		...
		// Find current celestial body info and color in config files, or if not found default neutral settings are used
		...
		// Void responsible for the rendering and calculating of the planetshine albedo lighting
		private void UpdateAlbedoLights() {
			...
			// Combining all previous albedo light modificators determined above to set the final intensity visible to the end user and in the client
			...
			foreach (GameObject albedoLight in albedoLights) {
				...
				// Spreads out the lights, in the case of multiple albedo light sources
				if (albedoLightsQuantity > 1) {
